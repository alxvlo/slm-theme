...
$allowed_views = ['dashboard', 'all-jobs', 'order-detail', 'memberships', 'clients', 'notifications', 'portfolio', 'account'];
...
$normalize_status = static function ($status): string {
  ...
  if ($s === '')
    return 'pending';
  if (in_array($s, ['completed', 'complete', 'delivered', 'fulfilled'], true))
    return 'completed';
  if (in_array($s, ['in-progress', 'in_progress', 'processing'], true))
    return 'in-progress';
  if (in_array($s, ['scheduled', 'scheduled_for'], true))
    return 'scheduled';
  if (in_array($s, ['cancelled', 'canceled', 'refunded'], true))
    return 'cancelled';
  if (in_array($s, ['open', 'active', 'confirmed', 'ghost'], true))
    return 'active';
  return 'pending';
};
...
$to_amount = static function ($value): float {
  if (is_array($value)) {
    if (isset($value['amount_cents']) && is_numeric($value['amount_cents']))
      return ((float) $value['amount_cents']) / 100.0;
    if (isset($value['cents']) && is_numeric($value['cents']))
      return ((float) $value['cents']) / 100.0;
...
if ($view === 'memberships' && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['slm_memberships_action'])) {
  ...
  if ($target_user_id > 0)
    $redirect_args['member_user_id'] = (string) $target_user_id;
  if ($memberships_q_redirect !== '')
    $redirect_args['memberships_q'] = $memberships_q_redirect;
  wp_safe_redirect(add_query_arg($redirect_args, $admin_portal_url));
  exit;
}
...
if ($memberships_q !== '')
  $memberships_base_args['memberships_q'] = $memberships_q;
if ($selected_member_user_id > 0)
  $memberships_base_args['member_user_id'] = (string) $selected_member_user_id;
...
<div class="portal-shell">
  <aside class="portal-sidebar">
    ...
    <nav class="portal-nav" aria-label="Admin Portal">
      <a class="<?php echo $view === 'dashboard' ? 'is-active' : ''; ?>" <?php echo $view === 'dashboard' ? ' aria-current="page"' : ''; ?>
        href="<?php echo esc_url(add_query_arg('view', 'dashboard', $admin_portal_url)); ?>">Dashboard</a>
      <a class="<?php echo in_array($view, ['all-jobs', 'order-detail'], true) ? 'is-active' : ''; ?>" <?php echo in_array($view, ['all-jobs', 'order-detail'], true) ? ' aria-current="page"' : ''; ?>
        href="<?php echo esc_url(add_query_arg('view', 'all-jobs', $admin_portal_url)); ?>">All Orders</a>
      <a class="<?php echo $view === 'memberships' ? 'is-active' : ''; ?>" <?php echo $view === 'memberships' ? ' aria-current="page"' : ''; ?>
        href="<?php echo esc_url(add_query_arg('view', 'memberships', $admin_portal_url)); ?>">Memberships</a>
      <a class="<?php echo $view === 'clients' ? 'is-active' : ''; ?>" <?php echo $view === 'clients' ? ' aria-current="page"' : ''; ?>
        href="<?php echo esc_url(add_query_arg('view', 'clients', $admin_portal_url)); ?>">Clients</a>
      <a class="<?php echo $view === 'notifications' ? 'is-active' : ''; ?>" <?php echo $view === 'notifications' ? ' aria-current="page"' : ''; ?>
        href="<?php echo esc_url(add_query_arg('view', 'notifications', $admin_portal_url)); ?>">Notifications</a>
      <a class="<?php echo $view === 'portfolio' ? 'is-active' : ''; ?>" <?php echo $view === 'portfolio' ? ' aria-current="page"' : ''; ?>
        href="<?php echo esc_url(add_query_arg('view', 'portfolio', $admin_portal_url)); ?>">Portfolio</a>
      <a class="<?php echo $view === 'account' ? 'is-active' : ''; ?>" <?php echo $view === 'account' ? ' aria-current="page"' : ''; ?>
        href="<?php echo esc_url(add_query_arg('view', 'account', $admin_portal_url)); ?>">Account</a>
    </nav>
    ...
    <div class="portal-wrap">
      ...
      <?php if ($view === 'dashboard'): ?>
        ...
        <section class="portal-actions">
          <a class="portal-action portal-action--primary"
            href="<?php echo esc_url(add_query_arg('view', 'all-jobs', $admin_portal_url)); ?>">
            ...
            <table class="table" aria-label="Recent Customer Orders">
              ...
              <tbody>
                <?php if ($orders_error !== null): ?>
                  <tr>
                    <td colspan="7">Unable to load recent orders: <?php echo esc_html($orders_error); ?></td>
                  </tr>
                <?php elseif (empty($recent_orders)): ?>
                  <tr>
                    <td colspan="7">No recent customer orders available yet.</td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($recent_orders as $order): ?>
                    ...
                    <tr>
                      <td>
                        <?php if (($order['raw_id'] ?? '') !== ''): ?>
                          <a
                            href="<?php echo esc_url(add_query_arg(['view' => 'order-detail', 'order_id' => $order['raw_id']], $admin_portal_url)); ?>"><?php echo esc_html($order['id']); ?></a>
                        <?php else: ?>
                      ...
                      <td><span
                          class="status-pill <?php echo esc_attr($status_class($order['status'])); ?>"><?php echo esc_html($status_label($order['status'])); ?></span>
                      </td>
      ...
      <?php if ($view === 'all-jobs'): ?>
        ...
        <section class="portal-tableCard" data-server-filtered="1">
          ...
          <form class="table-controls table-controls--server" method="get"
            action="<?php echo esc_url($admin_portal_url); ?>">
            <input type="hidden" name="view" value="all-jobs">

            <div class="table-control">
              <label for="orders_q">Search</label>
              <input id="orders_q" name="orders_q" type="search" value="<?php echo esc_attr($orders_search); ?>"
                placeholder="Search order, customer, service, address">
            </div>
          ...
          <div class="table-scroll">
            <table class="table" aria-label="All Customer Orders">
              ...
              <tbody>
                <?php if ($orders_error !== null): ?>
                  <tr>
                    <td colspan="9">Unable to load orders: <?php echo esc_html($orders_error); ?></td>
                  </tr>
                <?php elseif (empty($orders)): ?>
                  <tr>
                    <td colspan="9">No customer orders available yet.</td>
                  </tr>
                <?php elseif (empty($paged_orders)): ?>
                  <tr>
                    <td colspan="9">No matching orders for current filters.</td>
                  </tr>
                <?php else: ?>
                      ...
                      <td>
                        <?php if (($order['raw_id'] ?? '') !== ''): ?>
                          <a class="btn btn--secondary"
                            href="<?php echo esc_url(add_query_arg(['view' => 'order-detail', 'order_id' => $order['raw_id']], $admin_portal_url)); ?>">View</a>
                          <?php if ((float) ($order['due_amount'] ?? 0) > 0.009 && (string) ($order['payment_url'] ?? '') !== ''): ?>
                            <a class="btn btn--secondary" href="<?php echo esc_url((string) $order['payment_url']); ?>"
                              target="_blank" rel="noopener">Pay</a>
                          <?php endif; ?>
                        <?php else: ?>
          ...
          <?php if ($orders_error === null && !empty($orders)): ?>
            <div class="portal-pagination">
              <p class="portal-pagination__summary">
                ...
                of <?php echo esc_html((string) $total_filtered_orders); ?>
                order<?php echo $total_filtered_orders === 1 ? '' : 's'; ?>
              </p>
              <?php if ($total_order_pages > 1): ?>
                <div class="portal-pagination__actions">
                  <?php if ($orders_page > 1): ?>
                    <a class="btn btn--secondary"
                      href="<?php echo esc_url($all_jobs_page_url($orders_page - 1)); ?>">Previous</a>
                  <?php else: ?>
                    <span class="btn btn--secondary is-disabled" aria-disabled="true">Previous</span>
                  <?php endif; ?>

                  <span class="portal-pagination__page">Page <?php echo esc_html((string) $orders_page); ?> of
                    <?php echo esc_html((string) $total_order_pages); ?></span>
      ...
      <?php if ($view === 'memberships'): ?>
        ...
        <?php if ($membership_notice !== ''): ?>
          <section class="portal-tableCard">
            <div style="padding:12px 16px;">
              ...
              ?>
              <p class="portal-notice portal-notice--<?php echo esc_attr($notice_meta[0]); ?>">
                <?php echo esc_html($notice_meta[1]); ?></p>
            </div>
          </section>
        <?php endif; ?>
        ...
        <section class="portal-tableCard" data-server-filtered="1">
          ...
          <form class="table-controls table-controls--server" method="get"
            ...
            <div class="table-control">
              ...
              <input id="memberships_q" name="memberships_q" type="search" value="<?php echo esc_attr($memberships_q); ?>"
                placeholder="Search member, email, tier, term">
            </div>
            <div class="table-control table-control--action">
            ...
            <table class="table" aria-label="Memberships">
              ...
              <tbody>
                <?php if (empty($memberships_rows)): ?>
                  <tr>
                    <td colspan="8">No memberships found yet.</td>
                  </tr>
                <?php elseif (empty($paged_memberships)): ?>
                  <tr>
                    <td colspan="8">No matching memberships for this search.</td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($paged_memberships as $member_row): ?>
                    ...
                    <tr>
                      ...
                      <td>
                        <?php echo esc_html((string) (($member_row['term_label'] ?? '') !== '' ? $member_row['term_label'] : 'n/a')); ?>
                      </td>
                      ...
                      <td>
                        <a class="btn btn--secondary"
                          href="<?php echo esc_url(add_query_arg(['view' => 'memberships', 'member_user_id' => (string) ($member_row['user_id'] ?? 0)], $admin_portal_url)); ?>">View</a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                <?php endif; ?>
          ...
          <?php if (!empty($memberships_rows)): ?>
            <div class="portal-pagination">
              <p class="portal-pagination__summary">
                Showing
                <?php echo esc_html((string) $memberships_showing_from); ?>-<?php echo esc_html((string) $memberships_showing_to); ?>
                of <?php echo esc_html((string) $total_memberships); ?> membership
                record<?php echo $total_memberships === 1 ? '' : 's'; ?>
              </p>
              <?php if ($total_membership_pages > 1): ?>
                <div class="portal-pagination__actions">
                  <?php if ($memberships_page > 1): ?>
                    <a class="btn btn--secondary"
                      href="<?php echo esc_url($memberships_page_url($memberships_page - 1)); ?>">Previous</a>
                  <?php else: ?>
                    <span class="btn btn--secondary is-disabled" aria-disabled="true">Previous</span>
                  <?php endif; ?>
                  <span class="portal-pagination__page">Page <?php echo esc_html((string) $memberships_page); ?> of
                    <?php echo esc_html((string) $total_membership_pages); ?></span>
                  <?php if ($memberships_page < $total_membership_pages): ?>
                    <a class="btn btn--secondary"
                      href="<?php echo esc_url($memberships_page_url($memberships_page + 1)); ?>">Next</a>
                  <?php else: ?>
                    <span class="btn btn--secondary is-disabled" aria-disabled="true">Next</span>
                  <?php endif; ?>
                </div>
        ...
        <?php if (!empty($selected_member_detail) && is_array($selected_member_detail)): ?>
          ...
          <section class="portal-tableCard">
            <div class="portal-tableCard__head">
              <h2>Member Detail<?php if ($member_user instanceof WP_User): ?>:
                  <?php echo esc_html((string) $member_user->display_name); ?>    <?php endif; ?></h2>
              <a href="<?php echo esc_url(add_query_arg('view', 'memberships', $admin_portal_url)); ?>">Back to
                Memberships</a>
            </div>
            <div class="order-detail" style="padding:16px;">
              <div class="order-detail__summary">
                <article class="portal-card">
                  <p>Plan</p><strong><?php echo esc_html($member_plan_label); ?></strong>
                </article>
                <article class="portal-card">
                  <p>Term</p>
                  <strong><?php echo esc_html((string) (($member_sub['term_label'] ?? '') !== '' ? $member_sub['term_label'] : 'n/a')); ?></strong>
                </article>
                <article class="portal-card">
                  <p>Status</p>
                  <strong><?php echo esc_html(function_exists('slm_subscriptions_status_label') ? slm_subscriptions_status_label((string) ($member_sub['status'] ?? '')) : (string) ($member_sub['status'] ?? '')); ?></strong>
                </article>
                <article class="portal-card">
                  <p>Provider</p>
                  <strong><?php echo esc_html(strtoupper((string) ($member_sub['provider'] ?? ''))); ?></strong>
                </article>
              </div>
              ...
              <div class="order-detail__grid">
                ...
                <article class="portal-card">
                  ...
                  <form method="post"
                    action="<?php echo esc_url(add_query_arg(['view' => 'memberships', 'member_user_id' => (string) $member_user_id], $admin_portal_url)); ?>">
                    ...
                    <p><label for="credit_key"><strong>Credit</strong></label><br>
                      <select id="credit_key" name="credit_key" style="width:100%;">
                        <?php foreach ((function_exists('slm_member_credits_key_labels') ? slm_member_credits_key_labels() : []) as $credit_key => $credit_label): ?>
                          <option value="<?php echo esc_attr((string) $credit_key); ?>">
                            <?php echo esc_html((string) $credit_label); ?></option>
                        <?php endforeach; ?>
                      </select>
                    </p>
                    <p><label for="quantity_delta"><strong>Quantity Delta</strong></label><br>
                      <input id="quantity_delta" type="number" name="quantity_delta" value="1" step="1">
                    </p>
                    <p><label for="adjust_note"><strong>Note</strong></label><br>
                      <input id="adjust_note" type="text" name="adjust_note" value="" style="width:100%;">
                    </p>
                ...
                <article class="portal-card">
                  ...
                  <?php if ((string) ($member_sub['provider'] ?? '') === 'square'): ?>
                    ...
                    <form method="post"
                      action="<?php echo esc_url(add_query_arg(['view' => 'memberships', 'member_user_id' => (string) $member_user_id], $admin_portal_url)); ?>"
                      onsubmit="return confirm('Schedule Square cancellation for this member if eligible?');">
                      <?php wp_nonce_field('slm_memberships_action', 'slm_memberships_nonce'); ?>
                    ...
                    <?php if ($member_commitment_active): ?>
                      <p class="sub" style="margin:8px 0 0;">Commitment active until
                        <?php echo esc_html(date_i18n('M j, Y', (int) $member_sub['commitment_end'])); ?>.</p>
                    <?php endif; ?>
                  <?php else: ?>
                  ...
                  <?php else: ?>
                    <ul class="order-detail__items">
                      <?php foreach ($member_flags as $flag_row): ?>
                        <?php if (!is_array($flag_row))
                          continue; ?>
                        <li>
                          <strong><?php echo esc_html((string) ($flag_row['flag_type'] ?? 'flag')); ?></strong>:
                          <?php echo esc_html((string) ($flag_row['message'] ?? '')); ?>
                          <form method="post"
                            action="<?php echo esc_url(add_query_arg(['view' => 'memberships', 'member_user_id' => (string) $member_user_id], $admin_portal_url)); ?>"
                            style="display:inline-block; margin-left:8px;">
                            <?php wp_nonce_field('slm_memberships_action', 'slm_memberships_nonce'); ?>
                            <input type="hidden" name="slm_memberships_action" value="resolve-credit-flag">
                            <input type="hidden" name="member_user_id"
                              value="<?php echo esc_attr((string) $member_user_id); ?>">
                            <input type="hidden" name="flag_id"
                              value="<?php echo esc_attr((string) (int) ($flag_row['id'] ?? 0)); ?>">
                  ...
                  <table class="table" aria-label="Member Credit Ledger">
                    ...
                    <tbody>
                      <?php if (empty($member_ledger)): ?>
                        <tr>
                          <td colspan="6">No credit ledger transactions yet.</td>
                        </tr>
                      <?php else: ?>
                        <?php foreach ($member_ledger as $ledger_row): ?>
                          <?php if (!is_array($ledger_row))
                            continue; ?>
                          <tr>
      ...
      <?php if ($view === 'clients'): ?>
        <section class="portal-section">
          <h1>Clients</h1>
          <p class="sub">Manage WordPress client accounts and jump to the standard user editor when changes are needed.
          </p>
        </section>
        ...
        <section class="portal-tableCard" data-server-filtered="1">
          ...
          <form class="table-controls table-controls--server" method="get"
            ...
            <div class="table-control">
              ...
              <input id="clients_q" name="clients_q" type="search" value="<?php echo esc_attr($clients_q); ?>"
            ...
            <table class="table" aria-label="Client Accounts">
              ...
              <tbody>
                ...
                <?php elseif (empty($paged_clients)): ?>
                  <tr>
                    <td colspan="7">No matching client accounts for this search.</td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($paged_clients as $client_row): ?>
                    ...
                    <tr>
                      ...
                      <td>
                        <?php echo esc_html((string) (($client_row['phone'] ?? '') !== '' ? $client_row['phone'] : 'n/a')); ?>
                      </td>
                      <td>
                        <?php echo esc_html((string) (($client_row['brokerage'] ?? '') !== '' ? $client_row['brokerage'] : 'n/a')); ?>
                      </td>
                      <td><?php echo esc_html($registered_label !== '' ? $registered_label : 'n/a'); ?></td>
                      <td><?php echo esc_html($role_label); ?></td>
                      <td>
                        <a class="btn btn--secondary"
                          href="<?php echo esc_url((string) ($client_row['edit_url'] ?? '')); ?>">Edit User</a>
                        <a class="btn btn--secondary"
                          href="<?php echo esc_url((string) ($client_row['portal_url'] ?? '')); ?>" target="_blank"
                          rel="noopener">Open Portal</a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                <?php endif; ?>
          ...
          <?php if (!empty($clients_rows)): ?>
            <div class="portal-pagination">
              <p class="portal-pagination__summary">
                Showing
                <?php echo esc_html((string) $clients_showing_from); ?>-<?php echo esc_html((string) $clients_showing_to); ?>
              ...
              <?php if ($total_client_pages > 1): ?>
                <div class="portal-pagination__actions">
                  <?php if ($clients_page > 1): ?>
                    <a class="btn btn--secondary"
                      href="<?php echo esc_url($clients_page_url($clients_page - 1)); ?>">Previous</a>
                  <?php else: ?>
                    <span class="btn btn--secondary is-disabled" aria-disabled="true">Previous</span>
                  <?php endif; ?>

                  <span class="portal-pagination__page">Page <?php echo esc_html((string) $clients_page); ?> of
                    <?php echo esc_html((string) $total_client_pages); ?></span>
          ...
          <section class="portal-tableCard">
            <div style="padding: 12px 16px;">
              ...
              <?php elseif ($order_notice === 'completion-email-missing-email'): ?>
                <p class="portal-notice portal-notice--warn">Cannot send completion email because the order is missing a
                  customer email.</p>
              ...
              <?php elseif ($order_notice === 'completion-email-failed'): ?>
                <p class="portal-notice portal-notice--warn">Completion email could not be sent. Check mail configuration and
                  order details.</p>
              <?php endif; ?>
            </div>
          </section>
            ...
            <div class="order-detail">
              <div class="order-detail__summary">
                <article class="portal-card">
                  <p>Status</p>
                  <strong><span
                      class="status-pill <?php echo esc_attr($status_class((string) $selected_order['status'])); ?>"><?php echo esc_html($status_label((string) $selected_order['status'])); ?></span></strong>
                </article>
                <article class="portal-card">
                  <p>Total</p>
              ...
              <div class="order-detail__grid">
                ...
                <article class="portal-card">
                  <h2>Delivery</h2>
                  <p>Delivered:
                    <?php echo esc_html($format_datetime((string) ($selected_order['delivery_at'] ?? '')) ?: 'n/a'); ?></p>
                  <p>Appointment:
                    <?php echo esc_html($format_datetime((string) ($selected_order['appointment_at'] ?? '')) ?: 'n/a'); ?>
                  </p>
                  <?php if ((string) ($selected_order['delivery_url'] ?? '') !== ''): ?>
                    <a class="btn btn--secondary" href="<?php echo esc_url((string) $selected_order['delivery_url']); ?>"
                      target="_blank" rel="noopener">Open Delivery Link</a>
                  <?php endif; ?>
                </article>
                ...
                <article class="portal-card">
                  ...
                  <p>Payment Status:
                    <?php echo esc_html(ucwords(str_replace('-', ' ', (string) ($selected_order['payment_status'] ?? 'unknown')))); ?>
                  </p>
                  <?php if ((float) ($selected_order['due_amount'] ?? 0) > 0.009 && (string) ($selected_order['payment_url'] ?? '') !== ''): ?>
                    <a class="btn btn--secondary" href="<?php echo esc_url((string) $selected_order['payment_url']); ?>"
                      target="_blank" rel="noopener">Open Payment Link</a>
                  <?php endif; ?>
                </article>
                ...
                <article class="portal-card">
                  ...
                  <p>Customer Email:
                    <?php echo esc_html($completion_email_recipient !== '' ? $completion_email_recipient : 'n/a'); ?></p>
                  ...
                  <?php else: ?>
                    <p>Send the customer a completion update email from WordPress (delivery link is only included when fully
                      paid).</p>
                    <form method="post"
                      action="<?php echo esc_url(add_query_arg(['view' => 'order-detail', 'order_id' => (string) ($selected_order['raw_id'] ?? '')], $admin_portal_url)); ?>">
                      ...
                      <input type="hidden" name="order_id"
                        value="<?php echo esc_attr((string) ($selected_order['raw_id'] ?? '')); ?>">
      ...
      <?php if ($view === 'notifications'): ?>
        ...
        <?php elseif ($notifications_notice === 'test-failed'): ?>
          <section class="portal-tableCard">
            <div style="padding: 12px 16px;">
              <p class="portal-notice portal-notice--warn">Test notification failed. Check recipient email and mail delivery
                setup.</p>
            </div>
          </section>
        <?php endif; ?>
        ...
        <section class="portal-tableCard" data-server-filtered="1">
          ...
          <div class="notifications-actions">
            <form method="post"
              action="<?php echo esc_url(add_query_arg('view', 'notifications', $admin_portal_url)); ?>">
            ...
            <form method="post" action="<?php echo esc_url(add_query_arg('view', 'notifications', $admin_portal_url)); ?>"
              onsubmit="return confirm('Clear notification log?');">
              <?php wp_nonce_field('slm_notifications_action', 'slm_notifications_nonce'); ?>
          ...
          <form class="table-controls table-controls--server" method="get"
            ...
            <div class="table-control">
              ...
              <input id="notifications_q" name="notifications_q" type="search"
                value="<?php echo esc_attr($notifications_q); ?>" placeholder="Search subject, event, order, recipient">
            </div>
            ...
            <div class="table-control">
              ...
              <select id="notifications_status" name="notifications_status">
                ...
                <option value="skipped-disabled" <?php selected($notifications_status, 'skipped-disabled'); ?>>Skipped
                  (Disabled)</option>
                ...
                <option value="skipped-empty-body" <?php selected($notifications_status, 'skipped-empty-body'); ?>>Skipped
                  (Empty Body)</option>
                <option value="skipped-duplicate" <?php selected($notifications_status, 'skipped-duplicate'); ?>>Skipped
                  (Duplicate)</option>
                <option value="skipped-not-ready" <?php selected($notifications_status, 'skipped-not-ready'); ?>>Skipped
                  (Not Ready)</option>
              </select>
            </div>
            ...
            <div class="table-control">
              ...
              <select id="notifications_type" name="notifications_type">
                ...
                <option value="aryeo-webhook" <?php selected($notifications_type, 'aryeo-webhook'); ?>>Aryeo Webhook
                </option>
                <option value="account-created" <?php selected($notifications_type, 'account-created'); ?>>Account Created
                </option>
                <option value="manual-test" <?php selected($notifications_type, 'manual-test'); ?>>Manual Test</option>
                <option value="customer-submission" <?php selected($notifications_type, 'customer-submission'); ?>>
                  Customer Submission</option>
                <option value="customer-completion" <?php selected($notifications_type, 'customer-completion'); ?>>
                  Customer Completion</option>
              </select>
            </div>
            ...
            <table class="table" aria-label="Notification Log">
              ...
              <tbody>
                <?php if (empty($notification_rows)): ?>
                  <tr>
                    <td colspan="8">No notifications logged yet.</td>
                  </tr>
                <?php elseif (empty($paged_notifications)): ?>
                  <tr>
                    <td colspan="8">No matching notifications for current filters.</td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($paged_notifications as $row): ?>
                    ...
                    <tr>
                      ...
                      <td><span
                          class="notif-pill <?php echo esc_attr($status_slug); ?>"><?php echo esc_html($row_status !== '' ? $row_status : 'unknown'); ?></span>
                      </td>
                      <td>
                        <?php if ($row_order_id !== ''): ?>
                          <a
                            href="<?php echo esc_url(add_query_arg(['view' => 'order-detail', 'order_id' => $row_order_id], $admin_portal_url)); ?>">
          ...
          <?php if (!empty($notification_rows)): ?>
            <div class="portal-pagination">
              <p class="portal-pagination__summary">
                Showing
                <?php echo esc_html((string) $notifications_showing_from); ?>-<?php echo esc_html((string) $notifications_showing_to); ?>
                of <?php echo esc_html((string) $total_notifications); ?>
                notification<?php echo $total_notifications === 1 ? '' : 's'; ?>
              </p>
              <?php if ($total_notification_pages > 1): ?>
                <div class="portal-pagination__actions">
                  <?php if ($notifications_page > 1): ?>
                    <a class="btn btn--secondary"
                      href="<?php echo esc_url($notifications_page_url($notifications_page - 1)); ?>">Previous</a>
                  <?php else: ?>
                    <span class="btn btn--secondary is-disabled" aria-disabled="true">Previous</span>
                  <?php endif; ?>

                  <span class="portal-pagination__page">Page <?php echo esc_html((string) $notifications_page); ?> of
                    <?php echo esc_html((string) $total_notification_pages); ?></span>

                  <?php if ($notifications_page < $total_notification_pages): ?>
                    <a class="btn btn--secondary"
                      href="<?php echo esc_url($notifications_page_url($notifications_page + 1)); ?>">Next</a>
                  <?php else: ?>
                    <span class="btn btn--secondary is-disabled" aria-disabled="true">Next</span>
                  <?php endif; ?>
                </div>
      ...
      <?php if ($view === 'portfolio'): ?>
        <section class="portal-section">
          ...
          <p class="sub">Edit portfolio item titles, categories, and metrics displayed on the public portfolio page.</p>
        </section>

        <section class="portal-tableCard" id="pMgrWrap" data-no-filter="1">
          <div class="portal-tableCard__head">
            <h2>Portfolio Items</h2>
            <button class="btn" id="pMgrAddBtn" type="button" style="font-size:0.88rem;padding:8px 18px;">+ Add New
              Item</button>
          </div>

          <!-- Toolbar: search + filter -->
          <div class="pMgr-toolbar">
            <input class="pMgr-search" id="pMgrSearch" type="search" placeholder="Search portfolio items..."
          ...
          <div class="table-scroll">
            <table class="table pMgr-table" aria-label="Portfolio items">
              <thead>
                <tr>
                  <th>Thumbnail</th>
                  <th>Title</th>
              ...
              <tbody id="pMgrTbody"></tbody>
            </table>
          </div>

          <div style="padding:16px 0 4px;text-align:right;">
            <button class="btn btn--secondary" id="pMgrResetBtn" type="button"
              style="font-size:0.82rem;padding:7px 16px;border:1.5px solid var(--primary);color:var(--primary);">Reset to
              Defaults</button>
          </div>
        </section>

        <!-- Edit / Add Modal -->
        <div class="pMgr-modal-overlay" id="portfolio-modal-overlay" aria-hidden="true" style="display:none">
          <div class="pMgr-modal" id="portfolio-modal-card" role="dialog" aria-modal="true" aria-labelledby="pMgrModalTitle">
            <button class="pMgr-modal__close" id="pMgrModalClose" type="button" aria-label="Close">&times;</button>
            <h2 id="pMgrModalTitle" class="pMgr-modal__title">Edit Portfolio Item</h2>
            <div class="pMgr-modal__body">
              <input type="hidden" id="pMgrModalId">

              <div class="pMgr-field">
                <label class="pMgr-label" for="pMgrModalTitle_input">Project Title</label>
                <input class="pMgr-input" id="pMgrModalTitle_input" type="text" placeholder="e.g. 6000 on the River">
              </div>
              ...
              <div class="pMgr-field">
                <label class="pMgr-label" for="pMgrModalM1">Metric 1</label>
                <input class="pMgr-input" id="pMgrModalM1" type="text" placeholder="e.g. Sold in 4 days">
              </div>

              <div class="pMgr-field">
                <label class="pMgr-label" for="pMgrModalM2">Metric 2</label>
                <input class="pMgr-input" id="pMgrModalM2" type="text" placeholder="e.g. 14,200 views">
              </div>

              <div class="pMgr-field">
                <label class="pMgr-label" for="pMgrModalM3">Metric 3</label>
                <input class="pMgr-input" id="pMgrModalM3" type="text" placeholder="e.g. 60+ Inquiries">
              </div>

              <div class="pMgr-field pMgr-field--check">
                <input type="checkbox" id="pMgrModalFeatured">
                <label for="pMgrModalFeatured">Set as Featured Project (shows in hero on portfolio page)</label>
              </div>
            </div>
            <div class="pMgr-modal__footer">
              <button class="pMgr-modal__save" id="pMgrModalSave" type="button">Save Changes</button>
              <button class="pMgr-modal__cancel" id="pMgrModalCancel" type="button">Cancel</button>
            </div>
          </div>
        </div>
        ...
        <script>
          (function () {
            ...
            var items = loadItems();
            var nextId = items.reduce(function (m, i) { return Math.max(m, i.id || 0); }, 0) + 1;

            /* ── Refs ── */
            var tbody = document.getElementById('pMgrTbody');
            var search = document.getElementById('pMgrSearch');
            var filter = document.getElementById('pMgrFilter');
            var addBtn = document.getElementById('pMgrAddBtn');
            var resetBtn = document.getElementById('pMgrResetBtn');
            var overlay = document.getElementById('portfolio-modal-overlay');
            var modalTitle = document.getElementById('pMgrModalTitle');
            var mId = document.getElementById('pMgrModalId');
            var mTitle = document.getElementById('pMgrModalTitle_input');
            var mCat = document.getElementById('pMgrModalCat');
            var mImg = document.getElementById('pMgrModalImg');
            var mM1 = document.getElementById('pMgrModalM1');
            var mM2 = document.getElementById('pMgrModalM2');
            var mM3 = document.getElementById('pMgrModalM3');
            var mFeatured = document.getElementById('pMgrModalFeatured');
            var saveBtn = document.getElementById('pMgrModalSave');
            var cancelBtn = document.getElementById('pMgrModalCancel');
            var closeBtn = document.getElementById('pMgrModalClose');
            ...
            function renderTable() {
              ...
              var visible = items.filter(function (item) {
                ...
                var matchQ = q === '' || item.title.toLowerCase().indexOf(q) > -1 || item.category.toLowerCase().indexOf(q) > -1;
                return matchCat && matchQ;
              });
              tbody.innerHTML = '';
              visible.forEach(function (item, rowIdx) {
                ...
                tr.innerHTML =
                  ...
                  '<td>' +
                  '<button type="button" class="pMgr-action-btn pMgr-edit" data-id="' + item.id + '" style="border:1.5px solid #0d1b2a;color:#0d1b2a;background:transparent;padding:5px 13px;border-radius:6px;font-size:0.8rem;cursor:pointer;margin-right:6px;">Edit</button>' +
                  '<button type="button" class="pMgr-action-btn pMgr-delete" data-id="' + item.id + '" style="border:1.5px solid #DC2626;color:#DC2626;background:transparent;padding:5px 13px;border-radius:6px;font-size:0.8rem;cursor:pointer;">Delete</button>' +
                  '</td>';
                tbody.appendChild(tr);
              });
              /* Wire row buttons */
              tbody.querySelectorAll('.pMgr-edit').forEach(function (btn) {
                btn.addEventListener('click', function () { openEdit(parseInt(this.getAttribute('data-id'), 10)); });
              });
              tbody.querySelectorAll('.pMgr-delete').forEach(function (btn) {
                btn.addEventListener('click', function () { deleteItem(parseInt(this.getAttribute('data-id'), 10)); });
              });
            }

            search && search.addEventListener('input', renderTable);
            filter && filter.addEventListener('change', renderTable);
            ...
            function populateModal(item) {
              ...
              if (mM2) mM2.value = m[1] || '';
              if (mM3) mM3.value = m[2] || '';
              if (mFeatured) mFeatured.checked = item ? !!item.featured : false;
            }
